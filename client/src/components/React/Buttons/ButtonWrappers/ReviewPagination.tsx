...
export default function ReviewPagination({ setStep }) {
    ...
    return (
        <section className="w-full flex items-center mx-auto justify-center xs:gap-x-6">

            <button
                onClick={() => setStep(prev => prev - 1)}
                className="rounded-2xl group bg-white xs:w-24 xs:p-2 2xl:h-12 2xl:w-16 2xl:p-3 xs:text-sm hover:bg-white/20 transition-all duration-200 ease-in-out text-white font-light">
            ...
            <button
                onClick={() => setStep(prev => prev + 1)}
                className="rounded-2xl group bg-white xs:w-24 xs:p-2 2xl:h-12 2xl:w-16 xs:text-sm 2xl:p-3 hover:bg-white/20 transition-all duration-200 ease-in-out text-white font-light">
