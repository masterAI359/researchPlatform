        ...
        <div className="w-auto h-auto p-1 flex gap-x-0 items-center">
            ...
            <button
             ...
             ${paginateLimit ? 'bg-white/10 pointer-events-none' : 'pointer-events-auto'}`}>
                ...
                <svg className={`${paginateLimit ? 'text-white/10' : ''} p-1.5 text-black group-hover:text-white transition-all duration-200 ease-in-out`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="100%" height="100%">
            ...
            <button
                onClick={() => currentStory + 2 <= pages.length ? dispatch(incrementStory()) : dispatch(limitPagination(true))}
                className={`${paginateLimit ? 'bg-white/10 pointer-events-none' : 'pointer-events-auto'}
