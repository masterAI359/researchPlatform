    ...
    return (
        <div className={`${summaries.length > 1 ? 'block' : 'hidden'} w-auto h-auto p-0 md:p-1 flex gap-x-3 md:gap-x-0 items-center`}>
           ...
           `}>
            ...
            <div className="w-auto mx-auto xl:px-6 h-auto flex items-center justify-between gap-x-1 2xl:gap-2">
                {summaries?.map((item, index) => (
                    <div key={index} className={`cursor-pointer text-lg flex flex-col justify-center
                         rounded-lg p-2 w-6 h-8 md:w-8 md:h-fit hover:bg-white/20 hover:text-white transition-all ease-in-out duration-300
