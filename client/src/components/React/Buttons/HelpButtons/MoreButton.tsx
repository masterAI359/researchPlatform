...
export default function MoreButton({ article_url, showNotification, open, setOpen, context }) {
    ...
    return (
        <div className='w-full h-full xs:self-start md:self-center xs:mt-2 md:mt-0 relative'>
            {open && !showNotification ? <More context={context} key={article_url} article_url={article_url} setOpen={setOpen} />
                : null}
