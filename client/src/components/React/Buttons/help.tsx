...
export default function HelpButton() {
    ...
    return (
        <div className="mb-4 w-full self-end flex flex-row-reverse gap-2">
            <div
                onClick={() => setIsOpen(isOpen => !isOpen)}
                className="hover:cursor-pointer absolute ml-2 self-end rounded-full w-fit h-fit bg-white/10 flex-none">
            ...
            <AnimatePresence>
                {isOpen && <motion.div
                    ...
                    style={{ height: "300px", width: "700px", position: "absolute", marginTop: "10px" }}
                    className="bg-ebony z-40 rounded-xl"
                >
                    <p className="text-white">Help Button Test</p>
                </motion.div>}
            </AnimatePresence>
        </div>
