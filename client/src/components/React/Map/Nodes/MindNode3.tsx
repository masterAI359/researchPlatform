...
export default function MindNode3({ currentStep }) {
    ...
    return (
        ...
        ${currentStep === 2 ? 'scale-110' : null} bg-ebony 
         w-36 h-12 p-1 absolute md:top-40  md:right-6 flex items-center justify-beetween
      ${currentStep < 2 ? 'border border-2 border-button_gray' : 'border border-2 border-button_blue'} rounded-3xl transition-all duration-200 ease-in-out`}>
            <div className='flex gap-2 items-center w-full h-full box-border'>
                <motion.div
                    className="flex items-center justify-center rounded-full max-h-9 max-w-9 p-1  shrink-0 z-10 hover:cursor-pointer transition-all duration-300"
                    animate={{
                ...
                <div className='h-auto w-11/12 mx-auto'>
                    <h1 className="text-white xs:text-xs text-center font-light tracking tight">
                        Biases
                    </h1>
                </div>
            </div>
