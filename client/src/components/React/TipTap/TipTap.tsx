...
export default function TipTap({ setContent }) {
    ...
    return (<div className="w-full h-full box-border">
        <div className="control-group w-full flex px-1">
            <div className="button-group w-full mx-auto text-white flex gap-x-2 py-2 border-b border-white/10">
                <button
                    onClick={() => editor.chain().focus().toggleBulletList().run()}
                    className={editor.isActive('bulletList') ? 'is-active' : ''}
                >
                    <div className="max-h-8 max-w-8 p-1 bg-white/5 hover:bg-white/10 transition-all duration-200 ease-in-out rounded-md cursor-pointer">
        ...
        <EditorContent className="text-white text-lg focus:outline-none px-1 focus:border-none font-light tracking-tight cursor-text
             font-serif w-full mx-auto h-3/4 box-border prose"
            editor={editor} />
    </div>)
}
