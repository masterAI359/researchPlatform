...
export default function SavedArticles() {
    ...
    return (
        <section className="w-full border-l border-white/20 flex justify-end xs:gap-3 h-11/12 2xl:gap-6 mt-12">

            <article className="w-full flex flex-col gap-y-4 h-auto items-end">
                {userArticles.map((article: any, index: number) => {

                    const authorsShortened = limitArray(article.authors)
                    console.log(authorsShortened.length)
                                    ...
                                    <div className="flex flex-row-reverse justify-between h-full w-full items-center gap-x-4">
                                        <div className="flex flex-col h-full w-auto">
                                            <div className="w-fit h-full flex flex-col justify-end">
                                                <img className="2xl:max-w-80 2xl:min-w-80 2xl:max-h-44 object-cover rounded-2xl" src={article.image_url} />
                                            </div>
