...
export default function ChecksButton({ question, setterFunction, boolOption }) {
    const animationRef = useRef<any>(null)
    const dispatch = useDispatch()

    return (
        <button
            onClick={() => dispatch(setterFunction(boolOption))}
            className="flex items-center relative px-2 text-black hover:text-white
                ...
                rounded-lg transition-all duration-200 ease-inout group
                 "> {boolOption === true
            ...
            <div
             ...
             flex items-center justify-center">
                {question === boolOption ? <Lottie className="box-content absolute xs:right-0 xl:translate-x-1.5" lottieRef={animationRef} animationData={blueCheck} loop={false} autoPlay={false} style={{ height: "100%", width: "100%" }} /> : (
                    <div
